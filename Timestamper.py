...
# Dark and Light Mode
appearance = ctk.CTkSwitch(window, text='Change Theme', font=ctk.CTkFont('helvetica', size=20), fg_color='purple',
                            progress_color='orange', height=50, onvalue=0, offvalue=1, command=changemode)
appearance.grid(row=0, column=0, sticky='s')

# Input Field
timestamp_name = ctk.CTkEntry(window, placeholder_text='Title', font=ctk.CTkFont('helvetica', size=15), width=200,
                               height=50, corner_radius=500)
timestamp_name.grid(row=0, column=0, sticky='n', pady=150)

# Confirm Button
button = ctk.CTkButton(window, text="+", font=ctk.CTkFont('helvetica', size=30), command=count, width=50,
                        fg_color='black', hover_color=color, border_width=2, border_color='black', corner_radius=500)
button.grid(row=0, column=0, sticky='n', pady=230)

# Clear result Button
clear_result = ctk.CTkButton(window, text="Clear", font=ctk.CTkFont('helvetica', size=30), command=clear,
                              fg_color='black', hover_color=color, border_width=2, border_color='black', corner_radius=20)
clear_result.grid(row=0, column=0, sticky='n', pady=300)

# Result Frame
result = ctk.CTkScrollableFrame(window, width=370, height=1000, border_color='black',)
